package dsaquestions.BinarySearch;

public class RBS_Duplicate_Element {
    public static void main(String[] args) {
        int[] arr = {1,2,9,2,2,2,2,2};
        int t = 2;
        System.out.println(search(arr, t));
    }

    static int BS(int[] arr, int t, int l, int h) {
        while (l <= h) {
            int mid = l + (h - l) / 2;
            if (arr[mid] == t) {
                return mid;
            } else if (arr[mid] < t) {
                l = mid + 1;
            } else {
                h = mid - 1;
            }
        }
        return -1;
    }

    static int search(int[] arr, int t) {
        int p = findPivot(arr);
        //if you don't found the pivot
        //just do normal binary search
        if (p == -1) {
            return BS(arr, t, 0, arr.length - 1);
        }
        if (arr[p] == t)
            return p;

        if (t >= arr[0]) {
            return BS(arr, t, 0, p);
        } else {
            return BS(arr, t, p + 1, arr.length - 1);
        }
    }

    static int findPivot(int[] arr) {
        int l = 0;
        int h = arr.length - 1;
        while (l <= h) {
            int mid = l + (h - l) / 2;
            //Four cases over here
//            case : 1
            if (mid < h && arr[mid] > arr[mid + 1]) {
                return mid;
            }
//            case : 2
            else if (mid > l && arr[mid] < arr[mid - 1]) {
                return mid - 1;
            }
// If elements at middle, start , end are equal then just skip the duplicate
            if (arr[mid] == arr[l] && arr[mid] == arr[h]) {
//skip the duplicates
//NOTE : Check what if these elements at start and end were the pivot?
//Check if start is pivot
                if(arr[l] > arr[l+1]){
                    return l;
                }
                l++;
                if(arr[h] > arr[h-1]){
                        return h;
                }
                h--;
            }
            //left side is sorted , so pivot should be in right
            else if(arr[l] < arr[mid] || arr[l] == arr[mid]){
                l = mid+1;
            }else{
                h = mid-1;
            }
        }
        return -1;
    }
}
